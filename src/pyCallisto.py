...
class pyCallisto:
	...
	def spectrogram(self, option=3, xtick= 2, blevel=0, endPts= [False, False], figSize = (6,4), cmap=cm.jet, cBar=False, cBarOri='vertical', fontsize=25, vmax=40):
...
#		ax2 = plt.subplot2grid(shape=(8,8),loc=(5,0),rowspan=3,colspan=5)
	...
	def spectrogramWithGOES(self, tstart, tend, satellite_number=16, xtick=10, blevel=-10, vmax=40, 
							figSize=(14, 8), cmap=cm.jet, fontsize=14, grid=False,
							goes_colors=['red', 'darkblue'], save_path="spectrogram_with_goes.png"):
		"""
		Plot the spectrogram with GOES X-ray flux overlay.
		...
		Parameters:
			...
			xtick (int): Frequency of x-axis ticks in minutes (default = 10).
			blevel (int): Background level for spectrogram (default = -10).
			vmax (int): Maximum value for colormap (default = 40).
			figSize (tuple): Size of the figure (width, height) (default = (14, 8)).
			cmap: Colormap for the spectrogram (default = cm.jet).
			fontsize (int): Font size for labels and titles (default = 14).
			grid (bool): Show grid on GOES axis (default = False).
			goes_colors (list): Colors for GOES XRS-B and XRS-A [xrsb, xrsa].
		...
		try:
			...
			raise ImportError("This function requires sunpy and scipy. Please install them: pip install sunpy scipy")
		...
		# Plot spectrogram on left y-axis
		im = ax1.imshow(self.imageHdu.data, 
						extent=[xLims[0], xLims[1], yLims[0], yLims[1]], 
						aspect='auto', cmap=cmap, vmin=blevel, vmax=vmax)
		...
		ax1.set_ylabel('Frequency (MHz)', fontsize=fontsize, color='black')
		ax1.tick_params(axis='y', labelcolor='black')
		fig.autofmt_xdate()

		# Attempt to fetch and plot GOES data; on failure just save spectrogram
		try:
			...
			goes_truncated = goes_ts.truncate(tstart, tend)
			df_goes = goes_truncated.to_dataframe()

			# Create second y-axis for GOES data
			ax2 = ax1.twinx()

			# Plot GOES X-ray flux with smoothing (apply savgol_filter if enough points)
			if len(df_goes) >= 51:
				ax2.semilogy(df_goes.index, savgol_filter(df_goes['xrsa'], 51, 3),
							 label='GOES 0.5-4.0 Å', color=goes_colors[1], linewidth=2, alpha=0.8)
				ax2.semilogy(df_goes.index, savgol_filter(df_goes['xrsb'], 51, 3),
							 label='GOES 1.0-8.0 Å', color=goes_colors[0], linewidth=2, alpha=0.8)
			else:
				ax2.semilogy(df_goes.index, df_goes['xrsa'],
							 label='GOES 0.5-4.0 Å', color=goes_colors[1], linewidth=1, alpha=0.8)
				ax2.semilogy(df_goes.index, df_goes['xrsb'],
							 label='GOES 1.0-8.0 Å', color=goes_colors[0], linewidth=1, alpha=0.8)

			ax2.set_ylabel('X-ray Flux (Watts m$^{-2}$)', fontsize=fontsize, color=goes_colors[1])
			ax2.tick_params(axis='y', labelcolor=goes_colors[1])
			ax2.legend(loc='upper right', fontsize=max(8, fontsize-4))

			if grid:
				ax2.grid(True, alpha=0.3, which='both', axis='y')

			# Title when GOES is available
			title = 'Radio Spectrogram with GOES X-ray Flux Overlay\n' + self.imageHeader['DATE-OBS']
			plt.title(title, fontsize=fontsize+2)

		except Exception as e:
			# If any error occurs fetching or plotting GOES, just show spectrogram
			print("Warning: Could not fetch or plot GOES data:", e)
			title = 'Radio Spectrogram\n' + self.imageHeader['DATE-OBS']
			plt.title(title, fontsize=fontsize+2)
