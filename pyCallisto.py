...
# colorbar label set to intenity
# title is set from "content" of header
...
#configurations
clobber = True  #True/False overwrite the o/p files or not
...
def plot(fitsfile, imagepath, option=3, xtick= 3, blevel=0, endpts= [False, False], cmap=cm.jet, cbar=True, cbar_ori='vertical', fontsize=12):
	...
	Args:
		...
		xtick = x axis tick interval, default is every 3 minutes
	...
	if option == 1:
		...
		if cbar == True:
			ticks =list( np.linspace(blevel,datamax, 10).astype('int')) #calculate 10 ticks positins 
...
def jointimeaxis(fits1, fits2, ofile):
	...
	#if clobber flag is set to False, do not overwrite and print an warning
	if (not clobber and os.path.isfile(ofile)):
...
def subkg(infits, outfits):
	...
	# check for the inputs
	if not os.path.isfile(infits):
		print "file %s does not exists"%str(infits)
		return -1
	...
	print "size of the bintblfreqdata"
	print type(bintablehdu.data), len(bintablehdu.data)
	bintblfreqdata = bintablehdu.data[0][1]
	...
	if (not clobber and os.path.isfile(outfits)):
			print "file %s already exists, cannot overwrite, please choose a different output file name"
			hdus.close()
			#del hdus
			return -1
